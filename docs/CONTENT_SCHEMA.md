...
Les choix toneChoose et meaningChoose de la pédagogie sont représentés par kind: choice, avec une consigne et des labels qui donnent le profil de la tâche. wordOrder utilise des IDs de tuiles dans correctOrder ; fillBlank normalise le texte selon le contrat du moteur ; speaking donne une phrase de référence et des transcriptions acceptées pour la comparaison descriptive et la compatibilité des anciens événements ; la transcription seule ne note pas la prononciation dans l’UI actuelle. Un rapport `SpeechPronunciationService` est requis pour une réponse orale évaluée, sinon l’exercice peut être `skipped`. handwriting donne un caractère, un nombre de traits attendu et l’auto-évaluation ; flashcard référence un cardID sans recopier la carte.
